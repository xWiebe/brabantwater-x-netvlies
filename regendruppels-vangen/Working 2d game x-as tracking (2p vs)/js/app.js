let data;
let windowW = window.innerWidth; //check screen width
let windowH = window.innerHeight; //check screen height
...
let playerY = 800; //initialize playerY in the center of the screen
...
class FallingObject {
  ...
  update() {
    ...
    if (this.y > playerY - this.size / 2 && this.x > player1X.x - playerSize / 2 && this.x < player1X.x + playerSize / 2) {
      score1++;
      this.y = -this.size;
      this.x = random(windowW * (0.5 - fallingObjectArea), windowW * (0.5 + fallingObjectArea)); // objects only fall between the specified area
    }
    ...
    if (this.y > playerY - this.size / 2 && this.x > player2X.x - playerSize / 2 && this.x < player2X.x + playerSize / 2) {
      score2++;
      this.y = -this.size;
      this.x = random(windowW * (0.5 - fallingObjectArea), windowW * (0.5 + fallingObjectArea)); // objects only fall between the specified area
    }
...
function draw() {
  ...
  //Draw player 1
  image(player1Img, player1X.x - playerSize / 2, playerY - playerSize / 2, playerSize, playerSize);

  //Draw player 2
  image(player2Img, player2X.x - playerSize / 2, playerY - playerSize / 2, playerSize, playerSize);
  ...
  for (let i = 0; i < fallingObjects.length; i++) {
    ...
    //Check collision with player 1
    if (obj.y > playerY - obj.size / 2 && obj.x > player1X.x - playerSize / 2 && obj.x < player1X.x + playerSize / 2) {
      score1++;
      obj.y = -obj.size;
      obj.x = random(windowW * (0.5 - fallingObjectArea), windowW * (0.5 + fallingObjectArea)); // objects only fall between the specified area
    }

    //Check collision with player 2
    if (obj.y > playerY - obj.size / 2 && obj.x > player2X.x - playerSize / 2 && obj.x < player2X.x + playerSize / 2) {
      score2++;
      obj.y = -obj.size;
      obj.x = random(windowW * (0.5 - fallingObjectArea), windowW * (0.5 + fallingObjectArea)); // objects only fall between the specified area
    }
